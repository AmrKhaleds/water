                    ...
                    <div class="row">
                        <div class="col-8">
                            ...
                            <h6 class="card-text text-info"> @lang('back.total') @lang('back.bookings')
                                ...
                                = {{$bookings->where('status', 'inprocess')->sum(['total_amount'])}} @lang('back.L.E')</h6>
                            <h6 class="card-text text-danger"> @lang('back.total') @lang('back.bookings') @lang('back.canceled')
                                = {{$bookings->where('status','canceled')->count()}}
                                : @lang('back.total') @lang('back.price')
                                = {{$bookings->where('status','canceled')->sum(['total_amount'])}} @lang('back.L.E') </h6>
                        </div>
                        <div class="col-4">
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-nowrap table-centered mb-0">
                            ...
                            <tbody>
                            @foreach($bookings->where('status_ship','!=','canceled')->whereNull('vehicle_id') as $key => $booking)
                                <tr>
                                    <td>{{$key+1}}</td>
                                    <td>{{$booking->user->name}}</td>
                                    <td>
                                        {{--                                        <a href="https://wa.me/+2{{Str::replace('-','',$booking->client->profile->phone)}}?text=اهلا بك"--}}
                                        <a href="https://wa.me/+2{{str_replace('-','',$booking->user->profile->whatsapp)}}?text=اهلا بك" target="_blank">
                                            <div class="avatar-sm">
                                                    <span
                                    ...
                                    <td>{{str_replace('-','',$booking->user->profile->phone)}}</td>
                                    <td>{{$booking->user->profile->area->name}}</td>
                                    <td>{{$booking->count_orders}}</td>
                                    <td>{{$booking->total_amount}}@lang('back.L.E')</td>
                                    <td>
                                        <select name="" class="form-control" onchange="location = this.value;">
                                            ...
                                            @foreach($vehicles->whereNotNull('assign_car_id')->where('status','available') as $keyCount => $vehicle)
                                                <option
                                                        value="{{route('sales.assign',[$booking->id,$vehicle->id])}}" {{$booking->vehicle_id==$vehicle->id ? 'selected' : ''}}>{{$vehicle->car_name}}
                                    ...
                                    <td style="width: 100px">
                                        ...
                                        <a href="{{route('sales.edit',$booking->id)}}"
                                           class="btn btn-outline-warning btn-sm"
                                           title="@lang('back.edit')"><i class="fas fa-pencil-alt"></i></a>
                                        <a href="{{route('sales.show',$booking->id)}}"
                                ...
                                <div id="myModal{{$booking->id}}" class="modal fade" tabindex="-1" role="dialog"
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <form action="{{route('sales.trakings.destroy',$booking->id)}}" method="post">
                                            ...
                                            <table class="table table-nowrap table-centered mb-0">
                                                ...
                                                @foreach($car as $k => $value)
                                                    ...
                                                    <span
                                                        ...
                                                        <td>{{$value->user->profile->area->name}}</td>
                                                        <td>{{$value->count_orders}}</td>
                                                        <td>{{$value->total_amount}}@lang('back.L.E')</td>
                                                        <td style="width: 100px">
                                                            <div class="btn-group">
                                                                <button type="button"
                                                                        class="btn btn-{{$value->color_status}} dropdown-toggle waves-effect waves-light"
                                                                        data-bs-toggle="dropdown" aria-haspopup="true"
                                                                        aria-expanded="false">@lang('back.'.$value->status_ship)
                                                                    <i class="mdi mdi-chevron-down"></i></button>
                                                                <div class="dropdown-menu dropdown-menu-end"
                                                                     data-placement="top">
                                                                    <a class="dropdown-item text-primary"
                                                                       href="{{route('sales.status',[$value->id,'inprocess'])}}">
                                                                        <i class="fas fa-recycle"></i> @lang('back.inprocess')
                                                                    </a>
                                                                    <a class="dropdown-item text-info"
                                                                       href="{{route('sales.status',[$value->id,'inuploaded'])}}">
                                                                        <i class="fas fa-road"></i> @lang('back.inuploaded')
                                                                    </a>
                                                                    <a class="dropdown-item text-success"
                                                                       href="{{route('sales.status',[$value->id,'finished'])}}">
                                                                        <i class="fas fa-check-double"></i> @lang('back.finished')
                                                                    </a>
                                                                    <a class="dropdown-item text-warning"
                                                                       href="{{route('sales.undestroy',$value->id)}}">
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-nowrap table-centered mb-0">
                            ...
                            <tbody>
                            @foreach($bookings->where('status_ship','canceled') as $key => $booking2)
                                <tr>
                                    <td>{{$key+1}}</td>
                                    <td>{{$booking2->user->name}}</td>
                                    <td>{{str_replace('-','',$booking2->user->profile->phone)}}</td>
                                    {{--                                    <td>{{$booking2->client->name}}</td>--}}
                                    <td>
                                        <a href="https://wa.me/+2{{str_replace('-','',$booking2->user->profile->whatsapp)}}?text=اهلا بك" target="_blank">
                                    ...
                                    <td>{{$booking2->user->profile->area->name}}</td>
                                    <td>{{$booking2->count_orders}}</td>
                                    {{--                                    <td>@lang('back.'.$booking2->gallon_type)</td>--}}
                                    {{--                                    <td>@lang('back.'.$booking2->type)</td>--}}
                                    <td>{{$booking2->total_amount}} @lang('back.L.E')</td>
                                    <td>{{$booking2->note_canceled}}</td>
                                    <td style="width: 100px">
                                        <a href="{{route('sales.undestroy',$booking2->id)}}"
