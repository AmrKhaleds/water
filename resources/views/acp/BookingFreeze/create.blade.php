...
@section('title')
    @lang('back.create') @lang('back.bookings_freezers')
@endsection
@section('css')
    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-flex align-items-center justify-content-between">
                <h4 class="mb-0">@lang('back.create') @lang('back.bookings_freezers')</h4>

                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        <li class="breadcrumb-item active">@lang('back.create') @lang('back.bookings_freezers')</li>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        ...
                        <form method="post" action="{{route('booking.freezers.store')}}">
                            @csrf
                            <div class="row">
                                <div class="col-lg-6">
                                                    ...
                                                    @foreach($areas as $FromAreas)
                                                        <optgroup label="{{$FromAreas->name}}">
                                                            @foreach($FromAreas->children as $FromChildren)
                                                                <option {{old('from_area') == $FromChildren->id ? 'selected' : ''}} value="{{$FromChildren->id}}">{{$FromChildren->name}}</option>
                                                            @endforeach
                                                        </optgroup>
                                                    @endforeach
                                                </select>
                                                        ...
                                                        @foreach($areas as $ToAreas)
                                                            <optgroup label="{{$ToAreas->name}}">
                                                                @foreach($ToAreas->children as $ToChildren)
                                                                    <option {{old('to_area') == $ToChildren->id ? 'selected' : ''}} value="{{$ToChildren->id}}">{{$ToChildren->name}}</option>
                                                                @endforeach
                                                            </optgroup>
                                                        @endforeach
                                                    </select>
